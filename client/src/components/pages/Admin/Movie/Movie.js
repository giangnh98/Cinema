...
class Movie extends React.Component {
  ...
  fetchData = _.debounce((state) => {
    ...
    const createdBy = state.filtered.find(
      (filter) => filter.id === 'createdBy'
    );
    const updatedBy = state.filtered.find(
      (filter) => filter.id === 'updatedBy'
    );
    const url = `/api/movies/paginate?pageSize=${state.pageSize}&page=${
      state.page
    }
               &sortBy=${id}
  ...
  render() {
    return (
      <div>
        ...
        >
          <AddMovie
            selectedMovie={this.state.data.find(
